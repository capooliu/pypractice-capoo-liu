...
def download_dataset(data_path: str) -> None:
    ...
    if not os.path.exists(data_path) or not os.path.exists(dataset_dir):
        ...
        subprocess.run(["git", "clone", "--depth", "1", "https://github.com/nsojib/Bangla-Money-Dataset.git", dataset_dir], check=True)
...
def train_epoch(
    ...
    epoch_loss = running_loss / len(cast(Sized, loader.dataset))
...
def main() -> None:
    ...
    train_imgs, val_imgs, train_lbls, val_lbls = train_test_split(img_paths, labels, test_size=0.2, random_state=42)
    ...
    valid_loader = DataLoader(valid_set, batch_size=Config.BATCH_SIZE, shuffle=False, num_workers=Config.NUM_WORKERS, pin_memory=True)
    ...
    with torch.no_grad():
        for i, images in enumerate(tqdm(test_loader, desc="Predicting")):
            ...
            pred_val = trueclass(int(predicted.item()))
    ...
    # Export to CSV
    results_df = pd.DataFrame({"image": test_filenames, "class": prediction_classes})
